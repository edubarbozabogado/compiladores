...
void declaracion();  //Del analizador sintactico
void lista_declaracion(); //Del analizador sintactico
void unaDeclaracion(); //Del analizador sintactico
void lista_declaracion_prima(); //Del analizador sintactico
void lista_identificadores(); //Del analizador sintactico
void lista_identificadores_prima(); //Del analizador sintactico
void lista_declaracion_prima(); //Del analizador sintactico
...
void sigLex()
{
	...
	while((c=fgetc(archivo))!=EOF)
	{
		...
		else if (isalpha(c))
		{
			...
			if (t.pe->compLex==-1)
			{
                //es un identificador, se inserta en la tabla de simbolos                  
				sprintf(e.lexema,id);
				e.compLex=ID;
				insertar(e);
				t.pe=buscar(id);
...
void match(token expectedToken){
     ...
     if(t.compLex == expectedToken.compLex){
    	 ...
    	 sprintf(msg,"En match --> %s no esperado",t.pe->lexema);
         errorSintactico(msg);
     }
}

//lista declaracion prima
void lista_identificadores_prima(){
     token tListaIdentPrima;
     if(t.compLex == ','){    
        tListaIdentPrima.pe=buscar(t.pe->lexema);
        tListaIdentPrima.compLex=t.pe->compLex;
        match(tListaIdentPrima);
...
void lista_identificadores(){
     ...
     if(t.compLex == ID){    
        ...
        lista_identificadores_prima();
         ...
         sprintf(msg,"%s no esperado",t.pe->lexema);
         errorSintactico(msg);
         while(t.compLex!=EOF){
            sigLex();                       
            if(strcmp(t.pe->lexema,";")==0 || strcmp(t.pe->lexema,"EOF")==0){
                 //se reanuda desde aqui.
                 break;
            }
         }
     } 
...
void lista_declaracion_prima(){
    token tListaDecPrima;
    if(t.compLex != ';'){                 
        lista_declaracion();
    }
    
    //match
    tListaDecPrima.pe=buscar(t.pe->lexema);
    tListaDecPrima.compLex=t.pe->compLex;
    match(tListaDecPrima);  
